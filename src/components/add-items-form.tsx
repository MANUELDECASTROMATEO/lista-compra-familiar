    ...
    <section className="sticky top-0 z-20 w-full overflow-x-hidden border-b border-slate-200 bg-white/95 px-3 py-3 backdrop-blur sm:px-4">
      ...
      {showVoicePanel && (
        <div className="fixed inset-0 z-40 bg-slate-950/35">
          <div className="fixed inset-x-0 bottom-0 top-16 z-50 border-t border-slate-200 bg-white shadow-2xl sm:inset-x-[calc(50%-24rem)] sm:rounded-t-md">
            <div className="grid h-full grid-rows-[auto_minmax(0,1fr)_auto]">
              ...
              <div className="grid min-h-0 gap-3 overflow-y-auto px-4 py-4">
                <div className="grid gap-2">
                  <p className="text-xs font-semibold uppercase text-slate-500">Productos detectados</p>
                  {previewItems.length > 0 ? (
                    <div className="grid gap-2">
                      {previewItems.map((item, index) => (
                        <div key={`${item.normalizedName}-${index}`} className="flex min-h-12 items-center justify-between gap-3 rounded-md bg-emerald-50 px-3 py-2 text-base font-semibold text-emerald-950 ring-1 ring-emerald-200">
                    ...
                    <div className="rounded-md border border-dashed border-slate-300 px-4 py-10 text-center text-sm text-slate-600">Habla ahora para ver los productos aqui.</div>
                  )}
                </div>

                <details className="rounded-md border border-slate-200 bg-slate-50">
                  <summary className="cursor-pointer px-3 py-2 text-sm font-medium text-slate-700">Ver texto reconocido</summary>
                  <div className="border-t border-slate-200 p-3">
                    <p className="max-h-28 overflow-y-auto whitespace-pre-wrap text-sm text-slate-800">{voiceDraft || "Sin texto todavia."}</p>
                  </div>
                </details>
              </div>
